    ...
    function(coupons, $scope, $location, $routeParams, sessionService, Upload, endpoint) {
      ...
      coupons.get({
        ...
        if (data.status == "success") {
          ...
          _this.coupon.start= _this.coupon.start ? new Date(_this.coupon.start) : new Date();
          _this.coupon.expiry= _this.coupon.expiry ? new Date(_this.coupon.expiry) : new Date();
        } else {
          _this.notify = {
            message: data.statusMessage,
            status: data.status,
